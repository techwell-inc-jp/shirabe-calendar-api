...
export function renderLlmsFullTxt(): string {
  return [
    ...
    "    | Hub Pro          | ¥120,000  | 背骨。2+ API 横断 / SLA で自動推奨(cross-API backbone) |",
    "    | Hub Enterprise   | ¥280,000  | 機会対応のみ。custom SLA + 専用窓口 + dataset            |",
    "",
    "**始め方(階段)**: 住所単体なら ¥40,000 の Address Managed から始め、住所+人名+暦+法人番号の横断利用が増えると見積が自動で ¥120,000 の Hub Pro(背骨)を提示する。¥280,000 は大規模 MDM/CRM の機会対応のみ。",
    "",
