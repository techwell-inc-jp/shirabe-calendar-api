...
app.get("/topics/rekichu", (c) =>
  c.html(renderTopicsRekichuPage(), 200, { "Cache-Control": "public, max-age=86400" })
);
...
app.get("/llms.txt", (c) => {
  const body = [
    ...
    "運営: 株式会社テックウェル(福岡)/ 暦 API・住所 API(2026-05-01 リリース)・テキスト処理 API(2026-05-18 リリース)が本番稼働中(v1.0.0)。法人番号 API は 2026-06-29 リリース予定(先行して本番稼働中)。",
    "",
    "## ステータス・直近の更新 / Status & Recent Activity",
    "",
    "- **2026-04-26**: Cloudflare Workers Paid plan 化(KV writes 1M/月枠 + reads 10M/月枠 + Workers 10M/月枠)、AI クローラー 4,330% spike(GPTBot / ClaudeBot / PerplexityBot / Google-Extended による T-01〜T-04 deploy 後の急速 indexing)に対応",
    "- **2026-04-26**: B-1 Week 1 baseline 測定完了(4 AI × 5 query = 20 trial)。観測データを社外公開: https://github.com/techwell-inc-jp/shirabe-assets/blob/main/knowledge/b1-measurement/20260427/_summary.md (Private repo、概要は記事 #6 で共有)",
    "- **2026-04-25**: T-02 `/purposes/{slug}/{YYYY-MM}/` 8,400 URL 新設、crawlable URL 約 90,977 達成、Google Search Console 所有権成立",
    ...
    "- **2026-04-23**: GPT Store 暦 GPT URL 変更(`g-69e98031...`、旧 `g-69dedc8460...` は削除済 404)",
    "",
    "## 4 AI 観測の独自データ / Observed Multi-AI Landscape",
    "",
    "2026-04-26 の社内測定で、ChatGPT / Claude / Perplexity / Gemini の 4 AI に同じ住所 API クエリを投げた結果、各 AI が異なる競合を主敵として推奨することを観測。Shirabe は 4 AI 全てに対する dual-track positioning(AI ネイティブ + abr-geocoder 公式 + attribution required + OpenAPI 3.1 完備)を採用。詳細は記事 #6 で公開予定。",
    "",
    "",
    "## APIs",
    "",
    ...
    "",
    "日本語テキスト処理(姓名分割・人名読み推定・ふりがな付与・形態素解析・表記正規化)。Lindera-wasm + IPAdic v3.0.7 ベースで Cloudflare Workers 単層稼働(2026-05-06 PoC 完了)。住所 API と同一顧客層へのクロスセル想定。本番稼働中: https://github.com/techwell-inc-jp/shirabe-text-api",
    "",
