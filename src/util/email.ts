 ...
 *
 * binding 未設定(ローカル / オンボード前)の場合は送信せず false を返し、
 * 呼出側はフロー全体を壊さない(検証メールは届かないが anti-enumeration の
 * 汎用レスポンスは維持される)。
 */
 ...
 * 再発行検証メールを送信する。
 *
 * binding 未設定 / 送信失敗時は false を返す(例外を投げない = フローを壊さない)。
 *
